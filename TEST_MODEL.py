...
model = build_model()
model.load_weights(
    f'{os.path.join(sys.path[0])}//fair_memory_selection_0.5_m0.5_m1_exploit//decentralized_weights/weights_0_7.hdf5')

# Calculate Q-values for each (x1, x2) pair (replace this with your actual Q-values)
# x1_time_out = np.arange(2,12, 1)
# current_capacity = 0.15
# y1_bl = np.arange(95,100, 0.2)
# pa = 2.54
# (x1_time_out_, y1_bl_) = np.meshgrid(x1_time_out, y1_bl)
# x1_flattened = x1_time_out_.flatten()
# y1_flattened = y1_bl_.flatten()
#
# states1 = []
# number_of_points_in_mesh = len(x1_flattened)
# for index in range(number_of_points_in_mesh):
#     states1.append([x1_flattened[index], current_capacity, pa, y1_flattened[index]])
#
# action1 = []
...
#
# action1 = np.array(action1).reshape(x1_time_out_.shape[0], y1_bl_.shape[1])
#


x2_buffer_length = np.arange(50, 100, 0.5)
y2_current_capacity = np.arange(50, -0.5, -0.5)
tout = 12
pa2 = 2.48
...
# for index in range(number_of_points_in_mesh):
#     states3.append([tout3,x3_flattened[index],y3_flattened[index],bl3])
#
# action3 = []
# for i in range(number_of_points_in_mesh):
#     states3[i] = np.array(states3[i]).reshape([1, np.array(states3[i]).shape[0]])
#     v = np.argmax(model(states3[i]))
#     print("action model : ", v)
#     action3.append(v)
#
...
# ax.plot_surface(x1_time_out_, y1_bl_, action1, cmap='coolwarm')
